app['settings'] = {
    ...
    init: async function () {
        ...
        if (sys.mob === true) {
            tk.cb('b1 b2 lt', 'Toggle mobile UI', async function () {
                if (sys.mobui === true) {
                    ...
                    await fs.del('sys.webdeskbooted/mobile');
                }
        ...
        tk.cb('b1 b2 nodontdoit lt', 'Developer Mode', async function () {
            ...
            if (opt !== "true") {
                ...
                tk.cb(`b1`, 'Disable (reboot)', async function () {
                    ...
                    await fs.delfold('sys.webdeskbooted/');
                    await fs.write('sys.webdeskbooted/name', sys.name);
        ...
        tk.cb('b7', 'On', async function () {
            sys.seconds = true;
            await fs.write('sys.webdeskbooted/clocksec', 'true');
        }, title);
        tk.cb('b7', 'Off', async function () {
            sys.seconds = false;
            await fs.write('sys.webdeskbooted/clocksec', 'false');
        }, title);
        ...
        tk.cb('b1', 'Auto', async function () {
            fs.write('sys.webdeskbooted/lightdarkpref', 'auto');
            const killyourselfapplesheep = await fs.read('sys.webdeskbooted/color');
        ...
        tk.cb('b7', 'More', async function () {
            ...
            const ok = await fs.read('sys.webdeskbooted/cnotifurl');
            if (ok === "true") {
                const ok2 = await fs.read('sys.webdeskbooted/notifsrc');
                the.value = ok2;
            }
            tk.cb('b1', 'Close', () => ui.dest(menu), menu); tk.cb('b1', 'Save', async function () {
                try {
                    await wd.notifsrc(the.value, true);
                    await fs.write('sys.webdeskbooted/cnotifurl', 'true');
                } catch (error) {
        ...
        tk.cb('b1', 'Reset Colors', async function () {
            fs.del('sys.webdeskbooted/color');
            fs.del('sys.webdeskbooted/lightdark');
            fs.del('sys.webdeskbooted/lightdarkpref');
        ...
        tk.cb('b1 b2 lt', 'Change Username', function () {
            ...
            sys.socket.on("changeuserdone", async () => {
                sys.name = inp.value;
                await fs.write('sys.webdeskbooted/name', inp.value);
        ...
        tk.cb('b1 b2 lt', 'Change Password', function () {
            ...
            sys.socket.on("changepassdone", async (token) => {
                await fs.write('sys.webdeskbooted/token', token);
                await wd.reboot();
            });
        }, userPane);
        ...
        tk.cb('b1 b2 lt', 'Log out', function () {
            ...
            tk.cb('b1', 'Log out', async function () {
                await fs.del('sys.webdeskbooted/token');
                await fs.write('sys.webdeskbooted/name', 'User');
                wd.reboot();
            }, menu);
            tk.cb('b1', `Close`, () => ui.dest(dark), menu);
        }, userPane);
        ...
        tk.cb('b7', 'Big', async function () {
            wd.bgft();
            fs.write('sys.webdeskbooted/font', 'big');
        }, p2);
        tk.cb('b7', 'Normal', function () {
            wd.meft();
            fs.write('sys.webdeskbooted/font', 'normal');
        }, p2);
        tk.cb('b7', 'Small', function () {
            wd.smft();
            fs.write('sys.webdeskbooted/font', 'small');
        }, p2);
        ...
        tk.cb('b7', 'No chances', async function () {
            sys.filter = true;
            sys.nc = true;
            fs.write('sys.webdeskbooted/filter', 'nc');
        ...
        tk.cb('b7', 'Filter', async function () {
            sys.filter = true;
            sys.nc = false;
            fs.write('sys.webdeskbooted/filter', 'true');
        ...
        tk.cb('b7', 'Off', function () {
            sys.filter = false;
            sys.nc = false;
            fs.del('sys.webdeskbooted/filter');
        ...
        const blur1 = tk.cb('b1 b2', 'Disable', function () {
            fs.write('sys.webdeskbooted/blur', 'false');
            ui.cv('ui1', 'var(--ui2)');
            ui.cv('bl1', '0px');
            ui.cv('bl2', '0px');
        }, blurp);
        ...
        const blur2 = tk.cb('b1 b2', 'Enable', async function () {
            await fs.del('sys.webdeskbooted/blur');
    ...
    locset: {
        ...
        init: function () {
            ...
            tk.cb('b1 b2', 'Disable Location', async function () {
                await fs.write('sys.webdeskbooted/location.json', [{ city: 'Paris, France', unit: 'Metric', lastupdate: Date.now(), default: true }]);
