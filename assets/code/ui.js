var ui = {
    ...
    filter: function (inputText) {
        if (sys.filter === true) {
            // yeouchers (generated by chatgpt plz dont cancel me :fire:)
            const offensiveTerms = new Set([
                // General profanity
                'fuck', 'fucker', 'fucking', 'shit', 'bullshit', 'bitch', 'asshole', 'bastard', 'damn', 'dick', 'prick', 'cunt', 'twat',

                // Racial and ethnic slurs (generalized, examples only; research sensitivity for exact terms)
                'nigg', 'chink', 'spic', 'cracker', 'gook', 'kike', 'wetback', 'gyppo',

                // Homophobic and gender slurs
                'fag', 'faggot', 'dyke', 'tranny', 'shemale', 'homo',

                // Terms related to self-harm or violence
                'kill', 'murder', 'suici', 'selfharm', 'cutting', 'worthless', 'hopeless', 'die', 'death', 'harm', 'enditall',

                // Sexual harassment and assault
                'rape', 'rapist', 'molest', 'molester', 'incest', 'pedophile', 'pedo',

                // Derogatory terms related to disabilities or mental health
                'retar', 'retard', 'idiot', 'moron', 'lunatic', 'crazy', 'psycho',

                // Other miscellaneous offensive phrases or terms
                'worthless', 'loser', 'whore', 'slut', 'pig', 'scum', 'filth'
            ]);

            const words = inputText.split(/\b/);
            const filteredWords = words.map(word =>
                offensiveTerms.has(word.toLowerCase()) ? '[filtered]' : word
            );
...
var tk = {
    ...
    p: function (contents, classn, div) {
        ...
        fuck.innerHTML = ui.filter(contents);
        if (classn) {
    ...
    ps: function (contents, classn, div) {
        const fuck = document.createElement('p');
        fuck.innerText = ui.filter(contents);
        if (classn) {
            fuck.classList = classn;
        }
        div.appendChild(fuck);
    ...
    cb: function (classn, name, func, ele) {
        ...
        button.innerText = ui.filter(name);
        if (func) {
